...
static void calculate_wall_dimensions(t_ray_vars *ray, t_wall_texture_vars *wall_vars)
{
...
static void calculate_texture_params(t_game *game, t_ray_vars *ray, t_wall_texture_vars *wall_vars)
{
    if (ray->side == 0)
        wall_vars->wall_x = game->player.position.y + ray->wall_dist * ray->ray_dir_y;
    else
        wall_vars->wall_x = game->player.position.x + ray->wall_dist * ray->ray_dir_x;
...
static void render_wall_texture(t_game *game, t_ray_vars *ray)
{
    ...
    wall_vars.pos = (wall_vars.draw_start - WIN_HEIGHT / 2 + wall_vars.line_height / 2) * wall_vars.step;
    wall_vars.y = wall_vars.draw_start;
    while (wall_vars.y < wall_vars.draw_end)
    {
        int tex_y = (int)wall_vars.pos & (TEXTURE_SIZE - 1);
        wall_vars.pos += wall_vars.step;
        
        wall_vars.color = game->texture_buffer[wall_vars.dir][TEXTURE_SIZE * tex_y + wall_vars.tex_x];
...
void ray_casting(t_game *game)
{
    ...
    while (x < WIN_WIDTH)
    {
        ...
        if (ray.side == 0)
            ray.wall_dist = (ray.map_x - game->player.position.x + (1 - ray.step_x) / 2) / ray.ray_dir_x;
        else
            ray.wall_dist = (ray.map_y - game->player.position.y + (1 - ray.step_y) / 2) / ray.ray_dir_y;
