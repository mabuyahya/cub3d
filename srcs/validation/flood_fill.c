/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   flood_fill.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mabuyahy <mabuyahy@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/24 10:19:05 by mabuyahy          #+#    #+#             */
/*   Updated: 2025/07/24 10:19:06 by mabuyahy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	check_within_boundaries(t_scene *scene, int x, int y)
{
	if (x == (int)ft_strlen(scene->map->map_2d[y]) - 1)
		free_all_and_print_exit(scene, ERR_UNCLOSED_MAP);
	if (x == 0)
		free_all_and_print_exit(scene, ERR_UNCLOSED_MAP);
	if ((y == 0 || y == ft_strlen_2d(scene->map->map_2d) - 1))
		free_all_and_print_exit(scene, ERR_UNCLOSED_MAP);
	if (scene->map->map_lens[y - 1] < (x + 1))
		free_all_and_print_exit(scene, ERR_UNCLOSED_MAP);
	if (scene->map->map_lens[y + 1] < (x + 1))
		free_all_and_print_exit(scene, ERR_UNCLOSED_MAP);
	if (scene->map->map_2d[y - 1][x] == ' ' || scene->map->map_2d[y
		+ 1][x] == ' ' || scene->map->map_2d[y][x - 1] == ' '
		|| scene->map->map_2d[y][x + 1] == ' ')
		free_all_and_print_exit(scene, ERR_UNCLOSED_MAP);
}

void	flood_fill(t_scene *scene, int x, int y)
{
	if (x < 0 || x > (int)ft_strlen(scene->map->map_2d[y]) || y < 0
		|| y > ft_strlen_2d(scene->map->map_2d)
		|| scene->map->map_2d[y][x] == ' ' || scene->map->map_2d[y][x] == 0
		|| scene->map->map_2d[y] == NULL)
		return ;
	if (scene->map->map_2d[y][x] != '1')
	{
		check_within_boundaries(scene, x, y);
		return ;
	}
	flood_fill(scene, ++x, y);
	flood_fill(scene, --x, y);
	flood_fill(scene, x, ++y);
	flood_fill(scene, x, --y);
}

void	check_closeness(t_scene *scene)
{
	t_point	o;

	o.x = 0;
	o.y = 0;
	while (scene->map->map_2d[(int)o.y])
	{
		o.x = 0;
		while (scene->map->map_2d[(int)o.y][(int)o.x])
		{
			if (scene->map->map_2d[(int)o.y][(int)o.x] != '1')
			{
				flood_fill(scene, (int)o.x, (int)o.y);
			}
			o.x++;
		}
		o.y++;
	}
}
